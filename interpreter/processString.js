...
function process(parse) {
  if (parse.type == "bool") return parse.value;
  var left;
  var right;
  var result;
  if (parse.left) left = process(parse.left);
  if (parse.right) right = process(parse.right);
  if (parse.operator == "||") {
    result = false;
    if (left != undefined) result = result || left;
    if (right != undefined) result = result || right;
    return result;
